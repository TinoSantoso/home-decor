...
import { keysToVelocity } from '../src/lib/tour-controls';
...
describe('keysToVelocity', () => {
  it('returns a zero vector when no keys are held', () => {
    const v = keysToVelocity({}, 1, 3);
    expect(v).toEqual([0, 0, 0]);
  });

  it('moves forward (-Z) when W is held', () => {
    const v = keysToVelocity({ KeyW: true }, 1, 3);
  ...
  it('diagonal movement is not faster than axis-aligned movement (normalized input)', () => {
    const diagonal = keysToVelocity({ KeyW: true, KeyD: true }, 1, 3);
    const forward = keysToVelocity({ KeyW: true }, 1, 3);
  ...
  it('returns zero vector when opposite keys cancel out (W + S)', () => {
    const v = keysToVelocity({ KeyW: true, KeyS: true }, 1, 3);
    expect(v).toEqual([0, 0, 0]);
  });

  it('works correctly with zero deltaSec (no movement even with key held)', () => {
    const v = keysToVelocity({ KeyW: true }, 0, 3);
    expect(v).toEqual([0, 0, 0]);
  });

  it('ignores irrelevant keys in the map', () => {
    const v = keysToVelocity({ Space: true, KeyW: true }, 1, 3);
    // Same as just KeyW
    const wOnly = keysToVelocity({ KeyW: true }, 1, 3);
